...
for (const [file, url] of [['index.html', 'https://x.test/'], ['us.html', 'https://x.test/us.html']]) {
  ...
  {// TOP5(이상적 매수후보): 데이터 신뢰 게이트가 여기도 걸려야 한다.
   // 레이더에서 '진입 금지'인 종목이 이상적 후보로 나오면 두 목록이 모순된다.
   const t5 = w.document.getElementById('top5Panel').innerHTML;
   for (const tk of ['STALE', 'OLD', 'B130', 'CONFL'])
     t(!t5.includes('>' + tk + '<'), `${tk} 는 TOP5 에도 안 나온다(신뢰 게이트)`);
   t(/종목 제외/.test(t5), 'TOP5 가 몇 종목을 왜 뺐는지 밝힌다');
   // 두 목록의 관계를 줄마다 밝힌다 — "왜 서로 다른 종목이냐"에 대한 답
   t(/t5-rel/.test(t5), 'TOP5 각 줄에 레이더 관계 배지');
   t(/선취매 권역|선취매 아님/.test(t5), '배지에 권역 여부 또는 아닌 이유');
   t(/보는 순서는 레이더가 먼저/.test(t5), '보는 순서를 화면에 명시');}
