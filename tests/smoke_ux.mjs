...
for (const [file,dataFile,mk,otherHref] of [
  ...
  // 5.5) GPT 상주 지침 — 페이지 상수와 docs 문서가 어긋나면 실패
  {
    ...
    for (const [re, why] of [
      ...
      // 8차(2026-09-24) — 같은 경고를 여러 섹션에 반복하던 것을 한 곳으로 모았다.
      // 합치다가 빠지면 안 되는 것들이다.
      [/고점 대비 많이 빠졌으니 싸다/, '고점比·RS 를 싸다는 논거로 쓰지 않는다'],
      [/필터에 걸렸을 뿐/, '논거를 못 세우면 필터에 걸렸을 뿐'],
      [/가치함정/, '추정치 하향이면 가치함정'],
      [/V9\. 오버행/, '소형주 오버행 점검'],
      [/전제가 이 종목에선 성립하지 않는다/, '베어 케이스에 스크리너 전제 붕괴 포함'],
      [/V-1\.[\s\S]*확신도를 '하'로 고정/, '검색 불가면 확신도 하'],
    ]) t(re.test(g), `지침 규칙 유지 — ${why}`);
    // ChatGPT 맞춤 GPT 의 Instructions 칸은 8,000자까지만 받는다. 7차는 1만 3천 자라
    // 뒷부분(검증 절차·출력 형식·절대 규칙)이 잘릴 수 있었다 — 잘려도 아무도 모른다.
    t(g.length <= 8000, `지침이 맞춤 GPT 칸(8,000자)에 들어간다 (${g.length}자)`);
    ...
    // 같은 날 두 번 고치면 날짜만으로는 못 가른다 — '·8차' 처럼 회차를 붙일 수 있다
    t(/^\d{4}-\d{2}-\d{2}(·\d+차)?$/.test(ver||''), `지침에 버전 날짜가 있다 (${ver})`);
