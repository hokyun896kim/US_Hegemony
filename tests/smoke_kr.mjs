...
t(p.includes("헤게모니 트리 지침") && p.includes('지침이 설치돼 있지 않다면'),
  '지침 참조 + 미설치 폴백 포함');
// 배점을 바꿨는데 사용자가 지침을 다시 복사하지 않으면, 그 GPT 는 옛 지침의
// '고점比가 깊을수록 여력' 으로 계속 반대 논리를 편다. 지침 재복사는 사용자
// 손에 달렸지만 종목 프롬프트는 매번 새로 복사되므로, 이 한 줄이 옛 지침을
// 이긴다. 절차가 아니라 데이터 옆에 붙는 사실이라 '지침 일원화' 와 충돌하지
// 않는다 — 여기 분석 절차를 더 넣지는 마라.
t(p.includes('52주 고점 대비') && /싸다|여력/.test(p) && p.includes('배점은 0'),
  '고점比 데이터에 거꾸로 읽지 말라는 경고가 붙는다');
t(p.includes('현재가÷52주최고가−1'),
  '고점比 공식을 밝힌다 (툴마다 관행이 달라 부호·크기가 뒤집힌다)');
