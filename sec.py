...
------------------------------------
    ...
    공시 지연     중앙 33일 · 90분위 39일  (한국 DART 는 45~48일)
    음수 지연     0건 ← 분기말 전 공시는 있으면 안 된다

**지연 90분위가 396일로 나온 구간이 있었다.** 원인을 캐 보니 버그가 아니라
분사·회계연도 변경이다 — CEG·KVUE·SOLV 처럼 최근 분사한 회사는 분사 전
분기를 첫 10-K 에서 **소급해서** 태깅한다. 2021년 이후 120일 초과는 6,130건
중 146건(2.4%)이고 66종목에 몰려 있다.

그리고 이건 **가드가 제대로 도는 것**이다. 분사 전 독립 실적을 시장이 그때
알았을 리 없으므로, known_at 이 공시일까지 숨기는 게 맞다.

실제로 막히지 않는 것도 확인했다 — 평가 시점마다 8분기 이상 확보된 종목이
92~98% 다(2022-01 에 333/360, 2026-03 에 352/360).
